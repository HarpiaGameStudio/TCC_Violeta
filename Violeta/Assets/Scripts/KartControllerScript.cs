...
public class KartControllerScript : MonoBehaviour
{
    ...
    public Transform CenterOfMass; //Local do Centro de Massa do Kart
    public Transform PosFrente, PosTras; //Local onde serão disparados os power ups (frente/trás)
    ...
    public string tamanhoPersonagem = "Grande"; //Tipo de personagem
    ...
    void Update()
    {
        ...
        movimentarRodas();  //Move as meshes de acordo com os colliders
    }
    ...
    private void UsarPowerUp()
    {
        ...
        if (Input.GetButtonDown("Fire1")) //Se o power up foi disparado
        {
            switch (powerUpTipo)
            {
                #region Missel Comum
    ...
    {
        contadorVoltas.text = "Lap: " + lap.ToString() + "/" +gerenciador.Laps.ToString();
        textVelocimetro.text = "Velocidade: " + velAtual.ToString();
        colocacaoText.text = "Posição: " + posicao.ToString();
    }
